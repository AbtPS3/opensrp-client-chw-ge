...
public class GeUtil {
    ...
    public static void processEvent(AllSharedPreferences allSharedPreferences, Event baseEvent) throws Exception {
        if (baseEvent != null) {
            GeJsonFormUtils.tagEvent(allSharedPreferences, baseEvent);
            JSONObject eventJson = new JSONObject(GeJsonFormUtils.gson.toJson(baseEvent));
    ...
    public static boolean launchDialer(final Activity activity, final BaseGeCallDialogContract.View callView, final String phoneNumber) {
        if (ContextCompat.checkSelfPermission(activity, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {

            // set a pending call execution request
            if (callView != null) {
                callView.setPendingCallRequest(() -> GeUtil.launchDialer(activity, callView, phoneNumber));
            }
    ...
    public static void saveFormEvent(final String jsonString) throws Exception {
        ...
        Event baseEvent = GeJsonFormUtils.processJsonForm(allSharedPreferences, jsonString);
        GeUtil.processEvent(allSharedPreferences, baseEvent);
    }
    ...
    public static class CloseGeMemberFromRegister extends AsyncTask<Void, Void, Void> {
        private final String baseEntityId;

        public CloseGeMemberFromRegister(String baseEntityId) {
            this.baseEntityId = baseEntityId;
        }

        @Override
        protected Void doInBackground(Void... voids) {
            GeDao.closeGeMemberFromRegister(baseEntityId);
            return null;
        }
