...
public class NCUtils {
    ...
    public static org.smartregister.Context context() {
        return GeLibrary.getInstance().context();
    }
    ...
    public static SimpleDateFormat getSourceDateFormat() {
        return new SimpleDateFormat(GeLibrary.getInstance().getSourceDateFormat(), Locale.getDefault());
    }

    public static SimpleDateFormat getSaveDateFormat() {
        return new SimpleDateFormat(GeLibrary.getInstance().getSaveDateFormat(), Locale.getDefault());
    }
    ...
    public static ECSyncHelper getSyncHelper() {
        return GeLibrary.getInstance().getEcSyncHelper();
    }

    public static ClientProcessorForJava getClientProcessorForJava() {
        return GeLibrary.getInstance().getClientProcessorForJava();
    }
    ...
    public static void processHomeVisit(EventClient baseEvent, SQLiteDatabase database, String parentEventType) {
        try {
            Visit visit = GeLibrary.getInstance().visitRepository().getVisitByFormSubmissionID(baseEvent.getEvent().getFormSubmissionId());
            if (visit == null) {
                ...
                if (StringUtils.isNotBlank(parentEventType) && !parentEventType.equalsIgnoreCase(visit.getVisitType())) {
                    String parentVisitID = GeLibrary.getInstance().visitRepository().getParentVisitEventID(visit.getBaseEntityId(), parentEventType, visit.getDate());
                    visit.setParentVisitID(parentVisitID);
                }

                if (database != null) {
                    GeLibrary.getInstance().visitRepository().addVisit(visit, database);
                } else {
                    GeLibrary.getInstance().visitRepository().addVisit(visit);
                }
                    ...
                    for (Map.Entry<String, List<VisitDetail>> entry : visit.getVisitDetails().entrySet()) {
                        if (entry.getValue() != null) {
                            for (VisitDetail detail : entry.getValue()) {
                                if (database != null) {
                                    GeLibrary.getInstance().visitDetailsRepository().addVisitDetails(detail, database);
                                } else {
                                    GeLibrary.getInstance().visitDetailsRepository().addVisitDetails(detail);
                                }
                            }
                        }
                    }
